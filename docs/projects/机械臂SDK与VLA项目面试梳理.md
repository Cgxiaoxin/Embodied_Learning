...
> **关联**：[π0.5 完整实践](./π0.5双臂L6折纸VLA与SAC微调项目实践.md) · [02-VLA与RL进阶](../theory/02-VLA与RL进阶.md) · [机械臂运控专题](../../interview/topics/机械臂运控算法详解.md)

> 面向面试的四块核心经历：**7DOF 机械臂 SDK（运动学/动力学）**、**MoveIt 运动规划**、**Pinocchio/KDL 逆解框架**、**π0 折纸 VLA 微调**。每节含「项目背景 → 实现思路 → 面试话术 → 可能的追问」。  
> §四为面试口述版；训练日志、SAC 后训练、真机指标详见 [π0.5 完整实践](./π0.5双臂L6折纸VLA与SAC微调项目实践.md)。
...
> **完整版**：训练配置、SAC 后训练、Sim-to-Real 踩坑与真机成功率见 [π0.5 完整实践](./π0.5双臂L6折纸VLA与SAC微调项目实践.md)。本节为**面试 30 秒 + 架构速讲**。
